...
type Props = {
  /** Content of CardFooter. Can be anything, but see
    * [CardActions](../card-actions) and [CardBlock](../card-block). */
  children?: React$Node,
  /** If `expandable`, expand CardFooter immediately upon initialization */
  defaultIsOpen?: boolean,
  /** Display a trigger to expand/collapse CardFooter contents */
  expandable?: boolean,
  ...
  /** If `expandable`, label for expand/collapse trigger */
  triggerMessage?: (isOpen: boolean) => string,
  /** Available variants */
...
const styles = {
  ...
  /*
   * [1] A large Button, even with zero'd padding, is still a bit too large in
   *     this context. These styles allow the Button to shrink, but the Icon
 ...
 * CardFooter.
 */
class CardFooter extends Component<Props, State> {
  static defaultProps = {
    triggerMessage: isOpen => (isOpen ? 'Collapse contents' : 'Expand contents')
  };
  ...
  render() {
    const {
      ...
      expandable,
      title,
      triggerMessage = CardFooter.defaultProps.triggerMessage,
      ...restProps
    } = this.props;
    ...
    const ExpandCollapseIcon = isOpen ? IconExpandMore : IconExpandLess;
          ...
          <Title>
            ...
            {expandable && (
              <ToggleButton
                ...
                minimal
                iconStart={
                  <ExpandCollapseIcon title={triggerMessage(isOpen)} />
                }
              />
            )}
          </Title>
